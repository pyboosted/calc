...
describe("nullish coalescing operator (??)", () => {
  describe("basic functionality", () => {
    test("returns right operand when left is null", () => {
      ...
      expect(result).toEqual({ type: "number", value: toDecimal(42) });
    });
  ...
  describe("with variables", () => {
    test("returns default when variable is null", () => {
      ...
      expect(result).toEqual({ type: "number", value: toDecimal(100) });
    });
  ...
  describe("with different types", () => {
    ...
    test("works with objects", () => {
      ...
      if (result.type === "object") {
        ...
        if (a?.type === "number") {
          expect(fromDecimal(a.value)).toBe(1);
        }
      }
    });
  ...
  describe("comparison with || operator", () => {
    test("|| treats 0 as falsy, ?? does not", () => {
      ...
      expect(orResult.type).toBe("number");
      if (orResult.type === "number") {
        expect(fromDecimal(orResult.value)).toBe(42);
      }
      expect(nullishResult.type).toBe("number");
      if (nullishResult.type === "number") {
        expect(fromDecimal(nullishResult.value)).toBe(0);
      }
    });
    ...
    test("both treat null the same way", () => {
      ...
      expect(orResult).toEqual({ type: "number", value: toDecimal(42) });
      expect(nullishResult).toEqual({ type: "number", value: toDecimal(42) });
    });
  });
